...
Goal: create additional protocol over SWSACK to maintain a smoothed roundtrip time (RTT) and variance to adapt timeout to network conditions. Allows nodes to be network aware for changes during runtime.
...
* Methods  
  ...
  * \*retransmitTimer(seqNum)  
    ...
    * Otherwise   
      ...
      * retransmitTimeout \= min(maxRTO, 2\* retransmitTimeout)  
      * scheduleAfter(retransmitTimeout, retransmitTimer, (packet.seqNum))  
  * isInWindow(seqNum):  
  ...
  * \*updateTimeout(diff):  
    ...
    * SRRT \= .9\*SRRT \+ .1\*RRT  
    * retransmitTimeout \= clamp(maxRTO, minRTO, SRRT \+ 4\*Svar)
